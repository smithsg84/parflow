                ...
                int          buf_size)
{
  ...
  while (ptr != NULL)
  {
    ...
    /* 
     * If len (number of elements to unpack) is 0 don't need to 
     * do anything so skip creating MPI types to represent zero length
     * types.   Zero len caused problems with some versions of MPICH.
     */
    if (len > 0)
    {
      switch (ptr->type)
